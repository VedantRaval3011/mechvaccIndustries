...
export default function ProductsPage() {
  ...
  const filteredGroupedProducts = useMemo(() => {
    ...
    for (const [group, groupProducts] of Object.entries(groupedProducts)) {
      filtered[group] = groupProducts.filter((product) => {
        ...
        const priceMatch = (product.price ?? 0) >= priceFilter.min && (product.price ?? 0) <= priceFilter.max;
  ...
  const handlePriceFilterChange = (e: React.ChangeEvent<HTMLInputElement>, type: "min" | "max") => {
    const value = e.target.value === "" ? (type === "min" ? 0 : Infinity) : parseFloat(e.target.value);
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h1 className="text-5xl font-bold text-[var(--color-black)] mb-4">
            Our <span className="text-[var(--color-green)]">Premium</span> Products
          </h1>
            ...
            {showFilters && (
              ...
              >
                <div className="mt-6 pt-6 border-t border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <h3 className="text-sm font-medium text-gray-500 mb-3">PRICE RANGE</h3>
        ...
        {allFilteredProducts.length === 0 ? (
          ...
          >
            ...
            <h2 className="text-2xl font-semibold text-gray-700 mb-3">No Results Found</h2>
            <p className="text-lg text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any products that match your current filters.</p>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={resetFilters}
          ...
          >
            <AnimatePresence>
              {(activeGroup === "all" ? allFilteredProducts : filteredGroupedProducts[activeGroup] || []).map(
                (product, index) => (
                  <motion.div
                    key={product._id}
                    initial={{ opacity: 0, y: 20 }}
                    ...
                    <div className="relative overflow-hidden">
                      {product.displayImage && product.displayImage !== "" ? (
                        ...
                        <div className="w-full h-64 bg-gradient-to-r from-gray-100 to-gray-200 flex items-center justify-center">
                          ...
                          >
                            <img src="/no-image-placeholder.png" alt="No Image" className="h-20 w-20 opacity-40" />
                          </motion.div>
                        </div>
                      )}
                    ...
                    <div className="p-6">
                      ...
                      {product.seoKeywords && (
                        <div className="flex flex-wrap gap-2 mb-5">
                          {product.seoKeywords.split(',').slice(0, 3).map((keyword, idx) => (
                            <span 
                              key={idx} 
