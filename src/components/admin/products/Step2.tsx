...
import { Specification, Product } from '@/types/product';
...
export default function Step2({ productId, onNext }: Step2Props) {
  ...
  const onSubmit = async (data: { specifications: Specification[] }) => {
    setIsLoading(true);
    await fetch(`/api/products/step2/${productId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ specifications: data.specifications }),
    });
    onNext(productId, { specifications: data.specifications });
    setIsLoading(false);
  };

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 p-6 bg-white rounded-2xl shadow-lg">
      <h2 className="text-3xl font-bold text-gray-800 border-b pb-4">Specifications</h2>
