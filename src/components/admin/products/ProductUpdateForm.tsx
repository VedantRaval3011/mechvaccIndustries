...
export default function ProductUpdateForm({
  ...
  const onSubmit = async (data: UpdateFormData) => {
    ...
    try {
      ...
      let finalDisplayImageUrl = product.displayImage || "";
      if (data.displayImage instanceof File) {
        finalDisplayImageUrl = await uploadFileToCloudinary(data.displayImage, "products/display");
      }
      ...
      if (data.additionalImages && data.additionalImages.length > 0) {
        for (const item of data.additionalImages) {
          if (item.file instanceof File) {
            const uploadedUrl = await uploadFileToCloudinary(item.file, "products/additional");
