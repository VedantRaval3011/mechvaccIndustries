...
import { Query } from '@/types/product';
...
export default function Step3({ productId, onComplete }: Step3Props) {
  ...
  const onSubmit = async (data: { queries: Query[] }) => {
    setIsLoading(true);
    await fetch(`/api/products/step3/${productId}`, {
      method: 'PUT',
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 p-6 bg-white rounded-2xl shadow-lg">
      <h2 className="text-3xl font-bold text-gray-800 border-b pb-4">Queries</h2>
